...
# v6.1 - Final with All Features and Startup Fix
...
class RadioXBot:
    ...
    def run(self):
        ...
        while True:
            try:
                self.check_for_commands() # Check for commands from the UI
                now_local = datetime.datetime.now(pytz.timezone(TIMEZONE))
                ...
                if self.paused:
                    self.log_event("Script is paused by user command.")
...
@app.route('/status')
def status():
    # CORRECTED: This now reads from the single bot instance's state
    return jsonify({
...
def initialize_bot():
    ...
    if bot_instance.authenticate_spotify():
        ...
        # Don't send email on automatic startup, only when manually triggered
        threading.Thread(target=bot_instance.run_startup_diagnostics, kwargs={'send_email': False}).start()
